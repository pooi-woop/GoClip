...
- 使用 Viper 管理配置
- 使用 Zap 管理日志
- **异步处理切片字幕**：在生成切片的同时异步处理已经生成好的切片
- **长视频分段处理**：生成字幕之前自动给长视频分段
- **配置文件允许用户指定语言**：提高处理效率，避免AI自己识别
- **提示词文件单独管理**：方便用户更改提取高光的提示词
- **自动压制片头**：支持为切片添加片头
- **版权信息显示**：程序启动时显示版权信息，防止有人从二道贩子处购买
...
3. **长视频分段**：对于超过30分钟的视频，自动分割成25分钟的片段
4. **生成字幕**：使用 Whisper 生成 SRT 格式的字幕，支持用户指定语言
5. **提取高光**：从提示词文件中读取提示词，通过 OpenAI 兼容的 API（如 Qwen）从字幕中提取高光时刻，输出为 JSON 格式，包含标题、时间、内容
6. **生成切片**：根据提取的高光时间范围，使用内置的 ffmpeg 生成视频切片，同时异步处理已经生成好的切片
7. **为切片添加字幕**：对每个切片单独提取字幕并压制，确保每个切片只包含自己时间范围内的字幕
8. **压制片头**：如果配置了片头文件，为切片添加片头
9. **切片命名**：自动以高光标题命名切片
...
如果您觉得 GoClip 对您有帮助，欢迎给我买杯咖啡来支持项目的持续开发。[GitHub Funding](https://github.com/pooi-woop/GoClip/sponsors)
